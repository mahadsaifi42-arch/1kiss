...
if (!TOKEN) throw new Error("❌ DISCORD_TOKEN missing (set in Render Environment)");
if (!OWNER_ID) throw new Error("❌ OWNER_ID missing (set in Render Environment)");
...
function isExpired(ts, ms = 30000) { return Date.now() - ts > ms; }
...
async function lockServer(guild) {
  ...
  for (const [, ch] of guild.channels.cache) {
    try { await ch.permissionOverwrites.edit(everyone, { SendMessages: false }); } catch {}
  }
}
...
async function hideAllChannels(guild) {
  const everyone = guild.roles.everyone;
  for (const [, ch] of guild.channels.cache) {
    try { await ch.permissionOverwrites.edit(everyone, { ViewChannel: false }); } catch {}
  }
}
async function unhideAllChannels(guild) {
  const everyone = guild.roles.everyone;
  for (const [, ch] of guild.channels.cache) {
    try { await ch.permissionOverwrites.edit(everyone, { ViewChannel: null }); } catch {}
  }
}
...
async function deleteEmptyTextChannels(guild) {
  for (const [, ch] of guild.channels.cache) {
    ...
    try {
      ...
      if (msgs.size === 0) await ch.delete("Owner AI command: delete empty channels");
    } catch {}
  }
}
...
const commands = [
  ...
  new SlashCommandBuilder().setName("setup").setDescription("Setup AI channel + mod logs (Owner only)"),
...
async function playNext(guild, textChannel) {
  ...
  try {
    ...
    await logToModLogs(guild, `🎶 Music: Now playing ${next.title}`);
  } catch (e) {
...
Only in #ai-chat, owner can chat with Gemini AI.`
  ...
  if (interaction.commandName === "play") {
    ...
    if (!state.playing) {
      state.player.on(AudioPlayerStatus.Idle, async () => {
        const ch = guild.channels.cache.find(c => c.name === "ai-chat") || guild.systemChannel;
        if (!ch) return;
        await playNext(guild, ch);
      });

      const ch = guild.channels.cache.find(c => c.name === "ai-chat") || interaction.channel;
      await playNext(guild, ch);
    }
  }
  ...
  if (interaction.commandName === "queue") {
    const state = getGuildMusic(guild.id);
    if (!state.queue.length) return interaction.reply({ content: "📭 Queue is empty.", ephemeral: true });
...
function parseSimpleIntent(text) {
  ...
  if (t.includes("unhide all channels") || t.includes("show all channels")) return { intent: "unhide_all_channels" };
...
client.on("messageCreate", async (message) => {
  try {
    ...
    if (pending && isExpired(pending.createdAt)) pendingConfirmations.delete(message.author.id);
    ...
    if (simple.intent === "hide_all_channels") {
      pendingConfirmations.set(message.author.id, { action: "hide_all_channels", createdAt: Date.now() });
    ...
    if (simple.intent === "lock_server") {
      pendingConfirmations.set(message.author.id, { action: "lock_server", createdAt: Date.now() });
    ...
    if (simple.intent === "delete_empty_channels") {
      pendingConfirmations.set(message.author.id, { action: "delete_empty_channels", createdAt: Date.now() });
    ...
    if (simple.intent === "chat") {
      ...
      if (!genAI) return message.reply("🤖 AI disabled. Add GEMINI_API_KEY in Render Environment.");
